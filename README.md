...
One category takes over a quarter of all traffic and converts at roughly **3x the rest of the catalogue**. The store-wide 6.26% view→cart rate is a weighted average of a dominant product line and everything else, and describes neither. A further 30.2% of activity carries no `category_code` at all — close to 60% of traffic is either one product or unclassified.
...
- Confirm the cart defect at user level: purchases with no preceding cart event by the same user, and whether the affected categories share a product-page template
